...
setwd("C:/Users/ellys/Desktop/EAGLE/Permafrost")
...
setwd("C:/Users/farim/Documents/Permafrost")
Perma <- read.csv("df_total.csv")
Perma
...
paths <- "D:/rastertest/summer"

summerlis <- list.files(path= paths,
                        pattern='tif$',
                        full.names=TRUE,
                        recursive = TRUE,
                        include.dirs = FALSE)

rs <- lapply(summerlis, raster)

summerall <- stack(summerlis)

names(summerall) <- c("summer_2017",
                      "summer_2018",
                      "summer_2019",
                      "summer_2020",
                      "summer_2021",
...
forwaterm <- c(0, 2, 0, 2, 3, 1)
rclmat <- matrix(forwaterm, ncol=3, byrow=TRUE)
watersummer <- reclassify(summerall, rclmat)

#animation

anims <- tm_shape(watersummer) +
  tm_raster(palette = "Blues",legend.show = FALSE) +
  tm_facets(nrow = 1, ncol = 1) + 
  tm_layout(main.title = "Summer Water Class 2017-2022",
            main.title.position = "center",
            main.title.color = "brown",
            panel.labels = c("2017","2018","2019","2020","2021","2022"),
            panel.label.color = "black",
            legend.text.color = "black")

tmap_animation(anims,delay=70, "watersummer.gif")

###animationall summer

color <- c("#ae7346", "#7fc097", "#4680ae")
animsall <- tm_shape(summerall) +
  tm_raster(palette = color) +
  tm_facets(nrow = 1, ncol = 1) + 
  tm_layout(main.title = "Summer Dynamics 2016-2023",
            main.title.position = "center",
            main.title.color = "brown",
            legend.show = FALSE,
            panel.labels = c("2016","2017","2018","2019","2020","2021","2022"),
            panel.label.color = "black")

tmap_animation(animsall,delay=70, "allsummer.gif")

####Animation for winter####

# Load the raster

pathw <- "D:/rastertest/winter"

winterlis <- list.files(path= pathw,
                        pattern='tif$',
                        full.names=TRUE,
                        recursive = TRUE,
                        include.dirs = FALSE)

rw <- lapply(winterlis, raster)

winterall <- stack(winterlis)

names(winterall) <- c("winter_2016",
                      "winter_2017",
                      "winter_2018",
                      "winter_2019",
                      "winter_2020",
...
forwaterm <- c(0, 2, 0, 2, 3, 1)
rclmat <- matrix(forwaterm, ncol=3, byrow=TRUE)
waterwinter <- reclassify(winterall, rclmat)

#animation

animw <- tm_shape(waterwinter) +
  tm_raster(palette = "Blues",legend.show = FALSE) +
  tm_facets(nrow = 1, ncol = 1) + 
  tm_layout(main.title = "Winter Water Class 2016-2022",
            main.title.position = "center",
            main.title.color = "brown",
            panel.labels = c("2016","2017","2018","2019","2020","2021","2022"),
            panel.label.color = "black",
...
tmap_animation(animw,delay=70, "waterwinter.gif")


###animationall winter
color <- c("#ae7346", "#7fc097", "#4680ae")
animwall <- tm_shape(winterall) +
  tm_raster(palette = color) +
  tm_facets(nrow = 1, ncol = 1) + 
  tm_layout(main.title = "Winter Dynamics 2016-2022",
            main.title.position = "center",
            main.title.color = "brown",
            legend.show = FALSE,
            panel.labels = c("2016","2017","2018","2019","2020","2021","2022"),
...
tmap_animation(animwall,delay=70, "allwinter.gif")
